...
namespace Hell.Rune
{
    public class Attraction : TokenDrivenBehaviour<MasterAttack>, IRule<ActionToken>
    {
        public bool
            isPull = false;
        ...
        public void Resolution(ActionToken token)
        {
            ...
            foreach (Pawn pawn in Master.TargetPawns/*.OrderBy(p => p.Coord.Distance(Master.TargetCenter))*/)
            {
                ...
                Vector3 coordinate = (pawn.Coord.ToVector3(0.0f) - (token.Owner.Coord + token.Direction).ToVector3(0.0f)/* Master.TargetCenter.ToVector3(0.0f)*/);
                Direction direction = Coordinate.FromVector3(coordinate.normalized);
                if (isPull)
                    direction = direction.Invert();

                Debug.Log(direction);
                Coordinate result = Coordinate.FromVector3((coordinate + (((isPull) ? -1 : 1) * (coordinate.normalized * 1.1f))) + (token.Owner.Coord + token.Direction).ToVector3(0.0f)/*Master.TargetCenter.ToVector3(0.0f)*/);
